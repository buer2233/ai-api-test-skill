...
新增接口方法或 pytest 用例前，AI **在做任何调用、分析、扫描、抓包、写代码之前**，必须先检查用户是否已经提供以下 5 项必填信息。`[fixture]` 为选填，可省略或留空 `""`，不参与缺项判定。

### 必填信息清单与打回模板

新增任务校验发现缺项时，不要修改格式和任何内容，直接照抄以下模板返回：
...
- AI 必须逐项对照新增任务的 5 个必填字段，只认**真实文件路径 / 真实位置描述 / 真实用例名**。
...
AI 在 TodoWrite 首项必须显式记录任务类型、方式编号与 5 项必填任务信息，例如：

```text
[新增-方式2] 编写 [用例名]（用例文件：xxx，位置：xxx；本次无新增接口）
```
