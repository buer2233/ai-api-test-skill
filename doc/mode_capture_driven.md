...
8. **读勾选结果与落点校验**：
   ...
   - 新接口按 `pure_path` 前缀推荐并校验新增前置门禁中的 `[接口方法文件]`，冲突时反问用户。
   - 若新增前置门禁声明“当前用例无新增接口”，而草稿里仍有新接口 `[x]`：**必须打回**，让用户二选一（改新增前置任务信息或取消勾选新接口）。
...
12. **用例编写**：
   ...
   - 新接口方法写入新增前置门禁指定的 `[接口方法文件]` / `[接口方法位置]`；已实现接口按索引记录复用。
   - 新用例写入新增前置门禁指定的 `[接口用例文件]` / `[接口用例位置]`，并以 `[用例名]` 作为 docstring / 标题文本。
