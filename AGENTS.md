...
## 编码风格与命名约定
所有文件统一使用 UTF-8 编码。Markdown 内容保持简洁、直接，并与现有中文说明风格一致。Python 脚本遵循当前仓库既有风格：4 空格缩进、文件名使用 `snake_case`、函数职责单一、避免引入不必要依赖。生成产物和索引文件名称保持稳定，例如 `page_api_index.sqlite3`、`latest.jsonl`、`capture_selection.md`。
